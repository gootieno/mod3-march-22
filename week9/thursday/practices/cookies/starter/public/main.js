...
function storeTheme(themeName) {
  // Your code here
  const themeCookie = `themeName=${themeName}`;
...
function restoreTheme() {
  ...
  if (document.cookie) {
    if (grabCookie("themeName")) {
      setTheme(grabCookie("themeName"));
    }
  }
}
...
function clearTheme() {
  ...
  document.cookie = `themeName=;expires=Sun, 17 Sep 1989 00:00:00 GMT`;
  //   document.cookie = `themeName=;max-age=`;
}
...
function storeName(displayName) {
  // Your code here
  let nameCookie = "displayName=" + displayName;
...
function restoreName() {
  // Your code here
  if (document.cookie) {
    if (grabCookie("displayName")) {
      setInputValue("display-name", grabCookie("displayName"));
    }
  }
}

function grabCookie(cookieInput) {
  let array = document.cookie.split("; ");
  const foundCookie = array.find((cookie) => cookie.startsWith(cookieInput));
...
function clearName() {
  // Your code here
  document.cookie = `displayName=;max-age=`;
}
